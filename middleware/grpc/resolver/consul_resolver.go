...
func NewConsulBuilder(address string) resolver.Builder {
	...
	if err != nil {
		log.Fatal("grpc: create consul client error", err.Error())
		return nil
	}
...
func GetConsulGrpcConn(addr,serName string) (*grpc.ClientConn,error) {
	schema, err := GenerateAndRegisterConsulResolver(addr, serName)
	if err != nil {
		return nil,err
	}
	conn, err := grpc.Dial(fmt.Sprintf("%s:///%s", schema,serName), grpc.WithInsecure(), grpc.WithBalancerName(roundrobin.Name))
	if err != nil {
		return nil,err
	}
	return conn,nil
