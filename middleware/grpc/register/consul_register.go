...
func (cr *ConsulRegister) Register(info RegisterInfo) error {
	...
	if err = client.Agent().ServiceRegister(reg); err != nil {
		return fmt.Errorf("grpc: initial register service to consul error: %s", err.Error())
	}
	...
	if err != nil {
		return fmt.Errorf("grpc: initial register service check to consul error: %s", err.Error())
	}
...
func (cr *ConsulRegister) DeRegister(info RegisterInfo) error {
	...
	if err != nil {
		log.Println("grpc: deregister service error: ", err.Error())
	} else {
		log.Println("grpc: deregistered service from consul server.")
	}

	err = client.Agent().CheckDeregister(serviceId)
	if err != nil {
		log.Println("grpc: deregister check error: ", err.Error())
	}
